...
//    }
    ...
    public String show(Model model, @PathVariable("id") int id) {
        ...
        // Теперь findOne возвращает Optional
        Book book = bookService.findOne(id)
                .orElseThrow(() -> new EntityNotFoundException("Книга не найдена"));

        model.addAttribute("book", book);

        // Предполагается, что get возвращает Optional<Person>
        Optional<Person> ownerOpt = peopleService.get(book.getOwner() != null ? book.getOwner().getId() : null);

        ownerOpt.ifPresentOrElse(
                owner -> model.addAttribute("owner", owner),
                () -> model.addAttribute("people", peopleService.findAll())
        );
